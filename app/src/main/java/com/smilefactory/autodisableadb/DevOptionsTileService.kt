...
class DevOptionsTileService : TileService() {
    ...
    override fun onClick() {
        ...
        val enable = !DevOptions.isDeveloperOptionsEnabled(this)
        if (!DevOptions.setDeveloperOptionsOnly(this, enable)) {
            QuickTiles.toast(this, getString(R.string.need_write_secure))
            return
        }
    ...
    private fun render() {
        val on = DevOptions.isDeveloperOptionsEnabled(this)
        QuickTiles.apply(
            this,
            qsTile,
            active = on,
            label = getString(R.string.tile_developer_options),
